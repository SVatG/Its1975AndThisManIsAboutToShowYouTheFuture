...
{
	uint8_t byte = self->buffer[self->bufferPos];
	self->bufferPos = (self->bufferPos+1)%BUFFER_SIZE;
	if(self->bufferPos == 0) {
		self->bufferFillCallback(self->buffer+BUFFER_SIZE/2,BUFFER_SIZE/2);
	}
	if(self->bufferPos == BUFFER_SIZE/2) {
		self->bufferFillCallback(self->buffer,BUFFER_SIZE/2);
	}
	return(byte);
}
...
void InitRangeDecoder(RangeDecoder *self, void (*bufferFillCallback)(uint8_t*, uint32_t))
{
	...
	self->code=0;
	self->bufferFillCallback=bufferFillCallback;
	self->bufferPos = 0;
	self->bufferFillCallback(self->buffer,BUFFER_SIZE);
